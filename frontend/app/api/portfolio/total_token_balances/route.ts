...
interface TokenBalancesRequestBody {
  address: string;
  chains: string; // comma separated chain IDs, e.g. "1,56"
  excludeRiskToken?: '0' | '1'; // optional, default '0'
}
...
export async function POST(req: Request) {
  try {
    ...
    // Parse JSON body
    const body: TokenBalancesRequestBody = await req.json();

    const { address, chains, excludeRiskToken } = body;

    if (!address || !chains) {
      return new Response(
        JSON.stringify({ error: 'Missing required parameters: address and chains' }),
    ...
    if (excludeRiskToken !== undefined) {
      params.excludeRiskToken = excludeRiskToken;
    }

    const request_path = '/api/v5/dex/balance/all-token-balances-by-address';
